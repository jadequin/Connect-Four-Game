...
fun main() {
    ...
    while(!c4.isGameOver()) {
        c4 = if(c4.isP1Turn())
            ...
            c4.listMoves()[(readLine() ?: "0").toInt()]
//            c4.listMoves().random()
        println(c4)
        println()
    }

    //database output with pattern of: P1Hash:P2Hash:turn;Result
//    val dbText = ConnectFour.results.entries.joinToString(separator = ",") { "${it.key.first}:${it.key.second}:${it.key.third};${it.value}" }
